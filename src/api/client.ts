...
export function guidedApplicationMessage(applicationId: string, field: "vehicle" | "state" | "name", message: string, confirm = false) { return request<{ assistantMessage: string; extractedField: string | null; extractedValue: string | null; requiresConfirmation: boolean; needsClarification: boolean; mode: "llm" | "fallback"; saved: boolean; application: Application }>("/api/ai/application-message", { method: "POST", body: JSON.stringify({ applicationId, field, message, confirm }) }); }
